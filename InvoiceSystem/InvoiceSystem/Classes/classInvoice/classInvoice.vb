...
Public Class classInvoice
	Public Structure InvLocHeader
        ...
        Public h16_order_type As String
        Public h17_chargetxt As String
        Public h18_chargeamt As Double
        Public h19_id_banks As Nullable(Of Int64)
        Public h20_empcd As String
        'Public h16_is_proforma As String
        'Dim h16_chargetxt As String
        'Dim h18_chargeamt As Double
    End Structure
    ...
    Public Structure InvExpHeader
        ...
        Dim h52_id_banks As Nullable(Of Int64)
        ...
        Dim h44_is_proforma As Boolean
        Dim h45_entry_output_no As String
        Dim h51_entry_output_date As Date 'Sitthana 20190319
        Dim h46_entry_output_status As String
        Dim h47_boi_no As String
        Dim h48_benefit As String
        Dim h49_rm_no As String
        Dim h50_pending As String
    ...
    End Structure
    Public Function ValidateCompany(ByVal co As String, ByVal logempcd As String) As DataTable
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)

        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_export_validate_company"
    ...
    Public Function InvLocPacking(ByVal strPackNoCartNo As String, ByVal lngInvID As Long, ByVal lngInvOrd As Long)
        ...
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        ' comm.CommandText = "p_inv_local_packing" Disible By Neung
        comm.CommandText = "P_INV_LOCAL_PKG_select_packing"
        comm.Parameters.Clear()
    ...
    Public Function InvLocscPacking(ByVal strPackNoCartNo As String, ByVal lngInvID As Long, ByVal lngInvOrd As Long)
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_local_sc_packing"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@packno_cartno", strPackNoCartNo)
        comm.Parameters.AddWithValue("@invid", lngInvID)
        comm.Parameters.AddWithValue("@invord", lngInvOrd)
    ...
    Public Function InvLocSCLoad(Optional ByVal lngInvID As Long = 0, Optional ByVal strInvNo As String = "", Optional ByVal strDateFr As String = "", Optional ByVal strDateTo As String = "", Optional ByVal strCustCD As String = "") As DataTable
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_local_sc_select"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        comm.Parameters.AddWithValue("@invno", strInvNo)
        comm.Parameters.AddWithValue("@invdtfr", strDateFr)
    ...
    Public Function InvLocDetLoad(ByVal lngInvID As Long, Optional ByVal strInvNo As String = "") As DataTable
        ...
        comm.CommandText = "P_INV_LOCAL_PKG_select_inv_local_det"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        comm.Parameters.AddWithValue("@invno", strInvNo)
        Dim da As New SqlDataAdapter(comm)
        ...
        Return dt
    End Function
    Public Function InvLocDetscLoad(ByVal lngInvID As Long, Optional ByVal strInvNo As String = "") As DataTable
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_local_sc_det_select"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        comm.Parameters.AddWithValue("@invno", strInvNo)
        Dim da As New SqlDataAdapter(comm)
    ...
    Public Function InvLocSave(ByVal InvH As classInvoice.InvLocHeader, ByVal InvD_ADD As DataView, ByVal InvD_UPD As DataView, ByVal InvD_DEL As DataView, ByRef msgerr As String, ByRef invid As Long, ByRef invno As String) As Boolean
        ...
        With InvH
            ...
            comm.Parameters.AddWithValue("@order_type", .h16_order_type)
            comm.Parameters.AddWithValue("@chargetxt", .h17_chargetxt.Trim)
            comm.Parameters.AddWithValue("@chargeamt", .h18_chargeamt)
            comm.Parameters.AddWithValue("@id_banks", (New clsConfig).IsNull(.h19_id_banks, DBNull.Value))
            comm.Parameters.AddWithValue("@empcd", (New clsConfig).IsNull(.h20_empcd, DBNull.Value))
    ...
    Public Function InvLocCancel(ByVal lngInvID As Long, ByVal strEmpCD As String, ByRef msgerr As String) As Boolean
        Dim conn As New SqlConnection((New classConnection).connection)
        conn.Open()
        Dim tran As SqlTransaction = conn.BeginTransaction
        Dim comm As New SqlCommand("", conn)
    ...
    Public Function InvExpLabelDefault(ByVal StrInvno As String, pLabelType As String)
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "P_INV_EXPORT_LABEL_DEFAULT_PKG_select_default" ''"P_INV_EXPORT_LABEL_DEFAULT_PKG_select" Sitthana  20240313
        comm.Parameters.Clear()
        'comm.Parameters.AddWithValue("@invno", StrInvno)
        comm.Parameters.AddWithValue("@pinvno", StrInvno) ''Sitthana  20240313
        comm.Parameters.AddWithValue("@pLabelType", pLabelType)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
        da.Fill(dt)
        conn.Close()  'Sitthana 20190325
    ...
    Public Function InvExpLabelCopy(ByVal lngInvID As Long)
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_export_label_select_copy"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
    ...
    Public Function InvExpProductCopy(ByVal lngInvID As Long)
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_export_product_select_copy"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
    ...
    Public Function InvExpFreightCopy(ByVal lngInvID As Long)
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_export_freight_select_copy"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
    ...
    Public Function InvExpFreightMaster(ByVal lngInvID As Long)
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_export_freight_select_master"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
    ...
    Public Function InvExpLoad(Optional ByVal lngInvID As Long = 0, Optional ByVal strInvNo As String = "", Optional ByVal strDateFr As String = "", Optional ByVal strDateTo As String = "", Optional ByVal strCustCD As String = "") As DataTable
        ...
        '' For Test Data Only  'Sitthana 21/02/2018 
        'comm.CommandText = "p_combo_inv_export_showstatus_byyear"
        'comm.Parameters.AddWithValue("@year", "2015")
        '' End For Test Data Only  'Sitthana 21/02/2018

        comm.CommandText = "p_combo_inv_export_showstatus"   ' "p_inv_export_select"  'Sitthana 06/02/2018
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
        da.Fill(dt)
        conn.Close()  'Sitthana 20190325
    ...
    Public Function InvExpDetLoadCopy(ByVal lngInvID As Long) As DataTable
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_export_det_select_copy"
        comm.Parameters.Clear()
        comm.Parameters.AddWithValue("@invid", lngInvID)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
    ...
    Public Function InvExpSave(ByVal InvH As classInvoice.InvExpHeader,
     ByVal InvD_ADD As DataView, ByVal InvD_UPD As DataView, ByVal InvD_DEL As DataView,
     ByVal InvL_ADD As DataView, ByVal InvL_UPD As DataView, ByVal InvL_DEL As DataView,
     ByVal InvP_ADD As DataView, ByVal InvP_UPD As DataView, ByVal InvP_DEL As DataView,
     ByVal InvF_ADD As DataView, ByVal InvF_UPD As DataView, ByVal InvF_DEL As DataView,
     ByRef msgerr As String, ByRef invid As Long, ByRef invno As String) As Boolean
        ...
        With InvH
            ...
            comm.Parameters.AddWithValue("@grossamt", InvH.h07_grossamt.ToString)
            comm.Parameters.AddWithValue("@discamt", InvH.h08_discamt.ToString)
            ...
            comm.Parameters.AddWithValue("@id_banks", (New clsConfig).IsNull(.h52_id_banks, DBNull.Value))
            ...
            comm.Parameters.AddWithValue("@is_proforma", IIf(.h44_is_proforma, 1, 0))
            comm.Parameters.AddWithValue("@entry_output_no", .h45_entry_output_no.Trim)
            comm.Parameters.AddWithValue("@entry_output_date", .h51_entry_output_date) 'Sitthana 20190319
            comm.Parameters.AddWithValue("@entry_output_status", .h46_entry_output_status)
            comm.Parameters.AddWithValue("@boi_no", .h47_boi_no)
            comm.Parameters.AddWithValue("@benefit", .h48_benefit)
            comm.Parameters.AddWithValue("@rm_no", .h49_rm_no)
            comm.Parameters.AddWithValue("@pending", .h50_pending)
        End With
        ...
        Catch ex As Exception
            ...
            Return False
        End Try
        If (New clsConfig).IsNull(invid, 0) = 0 Then invid = Val(dt.Rows(0)("invid").ToString) 'Add By Neung 20180402
        'invid = Val(dt.Rows(0)("invid").ToString) 
        If (New clsConfig).IsNull(invno, "") = "" Then invno = dt.Rows(0)("invno").ToString 'Add By Neung 20180402
        'invno = dt.Rows(0)("invno").ToString
        ...
        For i = 0 To InvL_ADD.Count - 1
            With InvL_ADD
                ...
                comm.Parameters.AddWithValue("@line11", config.IsNull(.Item(i)("line11"), "").ToString.Trim) 'Sitthana 20240209
                comm.Parameters.AddWithValue("@line12", config.IsNull(.Item(i)("line12"), "").ToString.Trim) 'Sitthana 20240209
                comm.Parameters.AddWithValue("@line13", config.IsNull(.Item(i)("line13"), "").ToString.Trim) 'Sitthana 20240209
                comm.Parameters.AddWithValue("@line14", config.IsNull(.Item(i)("line14"), "").ToString.Trim) 'Sitthana 20240209
                comm.Parameters.AddWithValue("@line15", config.IsNull(.Item(i)("line15"), "").ToString.Trim) 'Sitthana 20240209
            End With
            da = New SqlDataAdapter(comm)
            dt = New DataTable
            Try
        ...
        i = 0
        comm.CommandText = "p_inv_export_freight_insert"
    ...
    Public Function InvLocSCSave(ByVal InvH As classInvoice.InvLocHeader, ByVal InvD_ADD As DataView, ByVal InvD_UPD As DataView, ByVal InvD_DEL As DataView, ByRef msgerr As String, ByRef invid As Long, ByRef invno As String) As Boolean
        Dim config As New clsConfig
        Dim conn As New SqlConnection((New classConnection).connection)
        conn.Open()
        Dim tran As SqlTransaction = conn.BeginTransaction
        ...
        comm.Transaction = tran
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_inv_local_sc_update"
        With InvH
            comm.Parameters.AddWithValue("@invid", .h01_invid)
            comm.Parameters.AddWithValue("@invno", .h02_invno)
            comm.Parameters.AddWithValue("@invdt", .h03_invdt)
            ...
            comm.Parameters.AddWithValue("@log_empcd", .h15_log_empcd)
            comm.Parameters.AddWithValue("@chargetxt", .h17_chargetxt.Trim)
            comm.Parameters.AddWithValue("@chargeamt", .h18_chargeamt.ToString)
            comm.Parameters.AddWithValue("@id_banks", (New clsConfig).IsNull(.h19_id_banks, DBNull.Value))
            comm.Parameters.AddWithValue("@empcd", (New clsConfig).IsNull(.h20_empcd, DBNull.Value))
        End With
        ...
        i = 0
        comm.CommandText = "p_inv_local_sc_det_update"
        ...
        i = 0
        comm.CommandText = "p_inv_local_sc_det_delete"
