Public Class frmDebitNoteProducts
#Region "Variables"
...
#Region "Customized Functions"
    Public Overridable Sub GenCombo2()
        ...
        Me.cboCustomer.DisplayMember = "cust_nameadd" 'name
    ...
    Public Overridable Sub ApproveData()
        ...
        If MessageBox.Show("Would you like to approve ?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.Yes Then Call (New classDebitNoteAR).Approve(Me.DocID, Me.UserInfo.UserID)
        txtStatus.Text = "APPROVED"
    End Sub

    Public Overridable Sub CancelData()
        If Not CanSave() Then Exit Sub
        If MessageBox.Show("Would you like to cancel ?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.Yes Then Call (New classDebitNoteAR).Cancel(Me.DocID, Me.UserInfo.UserID)
    ...
    Public Overridable Sub SumGrid(ByVal dt As DataTable)
        ...
        Dim dblVATAmt As Double = FormatNumber("0.00", 2, TriState.False, TriState.False, TriState.False)
        Dim dblNetAmt As Double = FormatNumber("0.00", 2, TriState.False, TriState.False, TriState.False)
        ...
        dblVATAmt = (dblPreTaxAmt * dblVAT) / 100
...
#Region "Events"
    ...
    Private Function CheckData() As Boolean
        For Each dgvr As DataGridViewRow In grdDetails.Rows
            If (New clsConfig).IsNull(dgvr.Cells("cboCurrency").Value, "").ToString.Trim = "" Then
        ...
        For Each dgvr As DataGridViewRow In grdDetails.Rows
            If (New clsConfig).IsNull(dgvr.Cells("cboCurrency").Value, "").ToString.Trim <> "THB" And (New clsConfig).IsNull(dgvr.Cells("exchange_rate").Value, 0) = 1 Then
                MessageBox.Show("ถ้าสกุลเงิน เป็นสกุลเงินต่างประเทศ Exchange Rate ต้อง มีค่ามากกว่า 1", "System Meassge", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1)
                Return False
            End If
        Next
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        'If grdDetails.CurrentRow.Cells("invtype").Value = "E" Then
        '    rptFileName = "rptDBNoteAR.rpt"
        'ElseIf grdDetails.CurrentRow.Cells("invtype").Value = "L" Then
        '    rptFileName = "rptDBNoteARLC.rpt"
        'End If 'Comment by Sitthana 20201208
        If cboInvtype.SelectedValue = "L" Then
            rptFileName = "rptDBNoteARLC.rpt"
        Else
            rptFileName = "rptDBNoteAR.rpt"
        End If 'Sitthana 20201208
    ...
    Private Sub cboDocNo_DropDownClosed(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboDocNo.DropDownClosed
        If CanChange() Then Call BindData(grdDetails, (New classDebitNoteAR).GetDocDetails(clsConfig.IsNull(cboDocNo.ComboBox.SelectedValue, 0), ""))
    End Sub
