...
Public Class frmInvoiceLocal
    ...
    Private Sub InitControl()
        ...
        '    End If
        'Next
        txtRemark.Text = "" 'Sittana 20200311
        Call EnabledControl(True)
        lngInvID = 0
        strInvNo = ""
        GroupBox2.Text = "Local Invoice"
        mcboCustomersBillToFlag.SelectedValue = DBNull.Value
        mcboCustomersShipToFlag.SelectedValue = DBNull.Value
        cboSalesMan.SelectedValue = DBNull.Value
    ...
    Private Sub EnabledControl(ByVal blnEnabled)
        ...
        mcboCustomersBillToFlag.Enabled = False
    ...
    Private Sub GenComboInvLoc()
		...
		cboInvNo.ComboBox.DataSource = objDB.InvLocLoad
	...
	Private Sub BindData(ByVal dt As DataTable)
        ...
        txtChargeAmt.Text = FormatNumber(dt.Rows(0)("chargeamt"), 2, TriState.False, TriState.False, TriState.True)
    ...
    Private Function CheckData() As Boolean
        ...
        If lblLocked.Visible Then
            ...
            Exit Function
        End If
        If (New clsConfig).IsNull(mcboCustomersBillToFlag.SelectedValue, "").ToString.Trim = "" Then
            ...
            Exit Function
        End If
        If (New clsConfig).IsNull(mcboCustomersShipToFlag.SelectedValue, "").ToString.Trim = "" Then
            MessageBox.Show("Please choose Delively to!!", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
            result = False
            Exit Function
        End If
        If grdInv.Rows.Count <= 1 Then
            ...
            Exit Function
        End If
        If (New clsConfig).IsNull(cboorder_type.SelectedValue, "") = "" Then
            MessageBox.Show("Please Select Order Type!!", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
    ...
    Private Sub LoadData(ByVal InvID As Long, ByVal InvNo As String)
        Dim objDB As New classInvoice
        Dim dt As New DataTable
        dt = objDB.InvLocDetLoad(InvID, InvNo)
        Call BindGrid(dt)
    ...
    Private Sub LoadPacking()
        ...
        Dim i As Integer = 0
        Dim j As Integer = 0
        dtNew = objdb.InvLocPacking(cboPackNo.SelectedValue, lngInvID, grdInv.Rows.Count)
        If dtNew.Rows.Count > 0 Then
            ...
            If config.IsNull(mcboCustomersBillToFlag.SelectedValue, "") = "" Then
                mcboCustomersBillToFlag.SelectedValue = config.IsNull(dtNew.Rows(0)("custcd"), "").ToString.Trim
            Else
                If mcboCustomersBillToFlag.SelectedValue <> config.IsNull(dtNew.Rows(0)("custcd"), "").ToString.Trim Then If MessageBox.Show("Customer is incorrect." & vbCrLf & "Would you like to change Customer Bill to?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) Then mcboCustomersBillToFlag.SelectedValue = config.IsNull(dtNew.Rows(0)("custcd"), "").ToString.Trim
            End If

            If config.IsNull(mcboCustomersShipToFlag.SelectedValue, "") = "" Then
                mcboCustomersShipToFlag.SelectedValue = config.IsNull(dtNew.Rows(0)("shipcd"), "").ToString.Trim
            Else
                If mcboCustomersShipToFlag.SelectedValue <> config.IsNull(dtNew.Rows(0)("shipcd"), "").ToString.Trim Then
                    If MessageBox.Show("Ship is incorrect." & vbCrLf & "Would you like to change Ship to ?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) Then
                        mcboCustomersShipToFlag.SelectedValue = config.IsNull(dtNew.Rows(0)("shipcd"), "").ToString.Trim
                    End If
                End If
            End If
            ...
            If config.IsNull(cboSalesMan.SelectedValue, "") = "" Then
                cboSalesMan.SelectedValue = config.IsNull(dtNew.Rows(0)("empcd"), "").ToString.Trim
            Else
                If cboSalesMan.SelectedValue <> config.IsNull(dtNew.Rows(0)("empcd"), "").ToString.Trim Then If MessageBox.Show("Sale Person is incorrect." & vbCrLf & "Would you like to change Sale Person ?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) Then mcboCustomersShipToFlag.SelectedValue = config.IsNull(dtNew.Rows(0)("empcd"), "").ToString.Trim
            End If
            'mcboCustomersBillToFlag.SelectedValue = config.IsNull(dt.Rows(0)("custcd"), "").ToString.Trim
            txtTermCondition.Text = config.IsNull(dtNew.Rows(0)("cond"), "").ToString.Trim
            mcboBanks.SelectedValue = dtNew.Rows(0)("id_banks")
            cboorder_type.SelectedValue = dtNew.Rows(0)("order_type")
            dt2 = grdInv.DataSource
            For i = 0 To dtNew.Rows.Count - 1
                dr = dt2.NewRow
                For j = 0 To dt2.Columns.Count - 1
                    dr(j) = dtNew.Rows(i)(dt2.Columns(j).ColumnName)
                Next
    ...
    Private Function CheckDataPacking(ByRef dt As DataTable) As Boolean
        ...
        Return True
    End Function
    Private Function getCustDesignInWord(pWord As String, pPosition As Byte)
        'Sitthana 14/11/2018
        Dim retWord As String = ""
        Dim DesignNo As String = ""
        Dim CustDesignNO As String = ""
        Dim DesignCol As String = ""
        Dim FirstPitePosion As Byte = 0
        Dim SecondPitePosion As Byte = 0
        Dim ThirdPitePosition As Byte = 0

        FirstPitePosion = InStr(pWord, "|")
        SecondPitePosion = InStr(FirstPitePosion + 1, pWord, "|")
        ThirdPitePosition = InStr(SecondPitePosion + 1, pWord, "|")
        If FirstPitePosion > 0 Then
            DesignNo = Mid(pWord, 1, FirstPitePosion - 1).Trim
            If SecondPitePosion > 0 Then
                If ThirdPitePosition > 0 Then
                    CustDesignNO = Mid(pWord, FirstPitePosion + 1, SecondPitePosion - FirstPitePosion - 1).Trim
                    DesignCol = Mid(pWord, FirstPitePosion + 1, SecondPitePosion - FirstPitePosion - 1).Trim
                Else
                    CustDesignNO = Mid(pWord, FirstPitePosion + 1, SecondPitePosion - FirstPitePosion - 1).Trim
                    DesignCol = Mid(pWord, SecondPitePosion + 1, Len(pWord) - SecondPitePosion).Trim
                End If
            Else
                CustDesignNO = Mid(pWord, FirstPitePosion + 1, Len(pWord) - FirstPitePosion).Trim
                DesignCol = ""
            End If

        Else
            DesignNo = Mid(pWord, 1, 20).Trim
            CustDesignNO = ""
            DesignCol = ""
        End If
    ...
    Private Sub LoadCharge()
        ...
        If txtItdesc.TextLength > 0 Then
            Dim designNo As String = getCustDesignInWord(txtItdesc.Text, 1) 'Sitthana 14/11/2018
            Dim CustDesign As String = getCustDesignInWord(txtItdesc.Text, 2)
            Dim DesignCol As String = getCustDesignInWord(txtItdesc.Text, 3)
            ...
            dt2 = grdInv.DataSource
            dr = dt2.NewRow
            dr("design_no") = txtDesignNo.Text 'designNo 'Sitthana 14/11/2018
            dr("itdesc") = txtItdesc.Text.Trim
            dr("cust_design") = txtCustDesign.Text ' CustDesign 'Sitthana 14/11/2018
            dr("col") = txtCol.Text.Trim ' DesignCol  'Sitthana 14/11/2018
            dr("grade") = txtGrade.Text.Trim
            dr("lineamt") = Val(txtUprice.Text) * Val(txtqty.Text.Trim)
            dr("linenetamt") = Val(txtUprice.Text) * Val(txtqty.Text.Trim)
            dr("pono") = txtCustPO.Text
            dr("sono") = txtsono.Text.Trim
            dr("qty") = IIf(txtqty.Text.Trim = "", 0, txtqty.Text.Trim) 'Sitthana 14/11/2018
            dr("uom") = cboUOM.SelectedValue
            dr("uprice") = txtUprice.Text
            dt2.Rows.Add(dr)

            grdInv.Rows(grdInv.Rows.Count - 2).Cells("design_no").Value = designNo  'Sitthana 14/11/2018

        End If
    End Sub

    Private Sub ClearChargeIssue()
        txtItdesc.Text = ""
        txtUprice.Text = ""
        txtqty.Text = ""
        cboUOM.SelectedValue = DBNull.Value
        txtCustPO.Text = ""
        txtsono.Text = ""
    End Sub
    ...
    Private Sub SumGrid()
        ...
        Dim dblDiscAmt As Double = 0 'txtDiscAmt.Text
        Dim dblPreTaxAmt As Double = 0
        Dim dblVat As Double = txtVat.Text
        Dim dblVatAmt As Double = 0
        Dim dblNetAmt As Double = 0
        'Add By Neung 26/02/2015
        Dim dblChargeAmt As Double = 0 'txtChargeAmt.Text
        'Begin Sitthana 20200225
        If txtDiscAmt.Text.Trim = "" Then
            dblDiscAmt = 0
        Else
            dblDiscAmt = Convert.ToDouble(txtDiscAmt.Text.Trim)
        End If

        If txtUprice.Text.Trim = "" Then
            dblChargeAmt = 0
        Else
            dblChargeAmt = Convert.ToDouble(txtUprice.Text.Trim)
        End If
        'End Sitthana 20200225
        ...
        'Dim dblVatAmt As Double = 0
        'Dim dblNetAmt As Double = 0
        'Dim dblChargeAmt As Double = txtChargeAmt.Text
        ''Dim dblChargeAmt As Double = Val(txtChargeAmt.Text)
        'For i = 0 To grdInv.Rows.Count - 1
        '    'Val(clsConfig.IsNull(grdInv.Rows(E.RowIndex).Cells("qty").Value, 0))
        '    'dblGrossAmt = dblGrossAmt + Format(Val(grdInv.Rows(i).Cells("linenetamt").Value), "#.#0")
        '    dblGrossAmt = dblGrossAmt + Format(Val(clsConfig.IsNull(grdInv.Rows(i).Cells("lineamt").Value, 0)), "#.#0")
        '    dblDiscAmt = dblDiscAmt + Format(Val(clsConfig.IsNull(grdInv.Rows(i).Cells("linediscamt").Value, 0)), "#.#0")
        'Next
        'dblGrossAmt = dblGrossAmt '+ dblChargeAmt
        'dblPreTaxAmt = dblGrossAmt - dblDiscAmt
        'dblVatAmt = Format((dblPreTaxAmt * dblVat) / 100, "#.#0")
        ''dblNetAmt = Round((dblPreTaxAmt * (100 + dblVat)) / 100, 2)
        'dblNetAmt = dblPreTaxAmt + dblVatAmt
    ...
    Private Function SaveData() As Boolean
        ...
        Dim dv_upd As New DataView(dt, "", "", DataViewRowState.ModifiedOriginal)
        ...
        invh.h16_order_type = (New clsConfig).IsNull(cboorder_type.SelectedValue, "")
        invh.h17_chargetxt = txtChargeIssue.Text.Trim
        'invh.h17_chargeamt = FormatNumber(txtChargeAmt.Text.Trim, 4, TriState.False, TriState.False, TriState.False) 'Sitthana 20200225
        invh.h18_chargeamt = FormatNumber(IIf(txtChargeAmt.Text.Trim = "", 0, txtUprice.Text.Trim), 4, TriState.False, TriState.False, TriState.False)
        ...
        If clsINV.InvLocSave(invh, dv_add, dv_upd, dv_del, msgerr, invid, invno) Then
            lngInvID = invid
            strInvNo = invno
            SaveData = True
        Else
    ...
    Private Sub txtDiscAmt_KeyDown(sender As Object, e As KeyEventArgs) Handles txtDiscAmt.KeyDown
    ...
    Private Sub BtnLoadCharge_Click(sender As System.Object, e As System.EventArgs) Handles BtnLoadCharge.Click
        ...
        If Not CheckDataLoadCharge() Then Exit Sub
    ...
    Private Function CheckDataLoadCharge() As Boolean
        ...
        If txtItdesc.Text.Trim = "" Then
            MessageBox.Show("ต้องมี ข้อความ Issue " & vbCrLf & "Qty Should be More then 0", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return False
        End If
    ...
    Private Sub tsmnInvoiceTHStandard_Click(sender As Object, e As EventArgs) Handles tsmnInvoiceTHStandard.Click
        If txtInvNo.Text.Trim.ToUpper = "" Then
            MessageBox.Show("You must entry invoice no first", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            printReport("rptInvLocal.rpt", "TH", "Local Invoice (TH)")
        End If
    End Sub

    Private Sub tsmnInvoiceTHNoPackingDetail_Click(sender As Object, e As EventArgs) Handles tsmnInvoiceTHNoPackingDetail.Click
        If txtInvNo.Text.Trim.ToUpper = "" Then
            MessageBox.Show("You must entry invoice no first", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            printReport("rptInvLocalNoPackDet.rpt", "TH", "Local Invoice (TH)")
        End If
    End Sub

    Private Sub tsmnInvoiceENStandard_Click(sender As Object, e As EventArgs) Handles tsmnInvoiceENStandard.Click
        If txtInvNo.Text.Trim.ToUpper = "" Then
            MessageBox.Show("You must entry invoice no first", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            printReport("rptInvLocal.rpt", "EN", "Local Invoice (EN)")
        End If
    End Sub

    Private Sub InvoiceENNoPackingDatail_Click(sender As Object, e As EventArgs) Handles InvoiceENNoPackingDatail.Click
        If txtInvNo.Text.Trim.ToUpper = "" Then
            MessageBox.Show("You must entry invoice no first", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            printReport("rptInvLocalNoPackDet.rpt", "EN", "Local Invoice (EN)")
        End If
    End Sub

    Private Sub printReport(pReportName As String, pLanguage As String, pTitle As String)
        ...
        rpt.SetParameterValue("@language", pLanguage)
    ...
    Private Sub btnSearchInvNo_Click(sender As Object, e As EventArgs) Handles btnSearchInvNo.Click
        Dim frm As New frmInvoiceLocalSearch
        frm.ShowDialog(Me)
        Call btnNew_Click(sender, e)
        Me.Cursor = Cursors.WaitCursor
    ...
    'Private Sub btnPrintInvoiceTH_Click(sender As Object, e As EventArgs)
    ...
    '    rpt.SetParameterValue("@custcd", "")
    '    'rpt.SetParameterValue("@chargetxt", txtChargeIssue.Text)
    '    'rpt.SetParameterValue("@chargeamt", txtChargeAmt.Text)
    ...
    'Private Sub btnPrintInvoiceEN_Click(sender As Object, e As EventArgs)
    '    If strInvNo = "" Then strInvNo = txtInvNo.Text.Trim.ToUpper
    '    If strInvNo.Length = 0 Then Exit Sub
    '    ' Const rptFileName = "rptInvLocal.rpt"
    '    Const rptFileName = "rptInvLocal.rpt"
