...
Public Class classPhoto
    ...
    Public Function GetsourceDocType(Optional ByVal Strsource_doc_type_code As String = "") As DataTable
    ...
    Public Function GetPhoto(Optional ByVal Strsource_doc_number As String = "") As DataTable
        Dim conn As New SqlConnection((New classConnection).connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_photo_select_doc_attachments"
        comm.Parameters.AddWithValue("@source_doc_number", Strsource_doc_number)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
        comm.Connection.Close()
        Try
