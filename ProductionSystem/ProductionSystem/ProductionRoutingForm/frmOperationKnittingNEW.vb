Public Class frmOperationKnittingNEW
    Dim oOpertionKnittingNew As New classOperationKnittingNEW
    ...
    Private Sub CheckVerifyUser()
        'Dim clsuser As New 
        Select Case classConnection.database
            Case "ColomboDB", "ColomboDBTest" 'CLA
                chkEsc.Text = "CLA"

                txtcustom_lot_number.ReadOnly = False
            Case Else 'ESH
                chkEsc.Text = "ESH"

                If clsuser.UserID = "TA" Or clsuser.UserID = "TON" Or clsuser.UserID = "CHET" Or clsuser.UserID = "SARUN" Or clsuser.UserID = "NUI" Then
                    txtcustom_lot_number.ReadOnly = False
                Else
                    txtcustom_lot_number.ReadOnly = True
                End If
        End Select
    ...
    Private Function GetDefectRoll(ByVal StrRollNo As String) As Boolean
        ...
        grdDefect.AutoGenerateColumns = False
        grdDefect.DataSource = (New classmfgOperationsStepKnitting).GetDefectRollNEW(strrollNo:=StrRollNo)
    ...
    Private Sub SetDefaultSubInventory(ByVal dt As DataTable)
        ...
        Try 'Sitthana 16/10/2018
            cbomtl_subinventory.SelectedValue = foundRows(0)(0)
        Catch ex As Exception
            MessageBox.Show("ไม่พบข้อมูล Sub Inventory ของ Warehourse " & cbomtl_warehouse.Text & vbCr &
                            "ที่ส่วนหนึ่งของข้อความ ของ sub inventory มีคำว่า GREIGE อยู่ด้วย " & vbCr &
                            Space(5) & "เช่น " & cbomtl_warehouse.Text.Trim & "_GREIGE" & vbCr &
                            "ดังนั้นกรุณาตรวจสอบ หรือเลือกใหม่ (โดยดูที่เลขม้วน)" _
    ...
    Private Sub GetComboLocation(ByVal Int64mtl_warehouse_id As Int64, ByVal Int64mtl_subinventory_id As Int64)
        ...
        cbomtl_location.DisplayMember = "location_name"
        cbomtl_location.ValueMember = "mtl_locations_id"
        Dim pMtlLocationsId As Nullable(Of Int64)
        pMtlLocationsId = oOpertionKnittingNew.GetDefaultLocationsId(Int64mtl_warehouse_id, Int64mtl_subinventory_id)
    ...
    Private Sub GetComboNewSubInventory(ByVal Int64mtl_warehouse_id As Int64)
        ...
        cbomtl_subinventory.DisplayMember = "subinventory_code"
        cbomtl_subinventory.ValueMember = "mtl_subinventory_id"
        Dim pMtlSubinventoryId As Nullable(Of Int64)
        pMtlSubinventoryId = oOpertionKnittingNew.GetDefaultSubInventoryId(Int64mtl_warehouse_id)
    ...
    Private Sub Loaddata(Optional ByVal strsystem_lot_number As String = Nothing)
        ...
        dtProductionLots = (New classOperationKnittingNEW).GetMfgOperationStepKnitting(IntMfgProductionLotsID:=Nothing,
                             StrSystemLotNumber:=strsystem_lot_number, IntmfgProductionStepsID:=Nothing,
    ...
    Private Sub Binddatatext(ByVal dt As DataTable)
        ...
        If dt.Rows.Count > 0 Then
            ...
            cbomtl_subinventory.SelectedValue = config.IsNull(dt.Rows(0)("mtl_subinventory_id"), Nothing)
            ...
            chkEsc.Checked = IIf(clsconfig.IsNull(dt.Rows(0)("custom_lot_number_gen"), "N") = "N", True, False)
            chkGMK.Checked = IIf(clsconfig.IsNull(dt.Rows(0)("custom_lot_number_gen"), "N") = "G", True, False)
            chkFactoryRollGMKFomat.Checked = IIf(clsconfig.IsNull(dt.Rows(0)("custom_lot_number_gen"), "NG") = "NG", True, False)
    ...
    Public Function Save() As Boolean
        ...
        Dim classOperationKnittingNEW As New classOperationKnittingNEW
        Dim Defect_Roll_Header As New classOperationKnittingNEW.Defect_roll_Header
        ...
        If classOperationKnittingNEW.SavaData(mfg_production_lots:=mfg_production_lots, Defect_Roll_Header:=Defect_Roll_Header _
    ...
    Private Sub PrintBarcodeNew() '08/10/2025 John
        Dim clsConfig As New clsConfig
        Dim rptFileName = "rptGreigeBarcodeNew.rpt"
        'If clsuser.ReportPath = "" Then clsuser.ReportPath = clsConfig.ReportPath
        'clsUser.ReportPath = "C:\Users\DELL\Desktop\GemmaKnits\"
        If Not clsConfig.CheckReport(clsuser.ReportPath, rptFileName) Then Exit Sub
        Dim frm As New frmReport
    ...
    Private Sub ApplyData()
        ...
        'If CheckDuplicate(Me.cbodefect_code.SelectedValue.ToString, dt2) Then
        ...
        If txtMTSStart.Text = "" Then
            txtMTSStart.Text = 0
        End If

        If txtLengthDefect.Text = "" Then
            txtLengthDefect.Text = 0
        End If
        ...
        For j = 0 To dt2.Columns.Count - 1
            ...
            dr("length_start") = Me.txtMTSStart.Text
            dr("length_defect") = Me.txtLengthDefect.Text
            dr("bar_no") = Me.txtBarNo.Text
            dr("guide_no") = Me.txtGuideNo.Text
            dr("defect_remark") = Me.txtDefectRemark.Text
    ...
    Private Function CheckDuplicate(ByVal defect_code As String, ByVal dt As DataTable) As Boolean
        If dt.Rows.Count > 0 Then
            Dim i As Integer = 0
    ...
    Private Sub SaveGreigeIN()
        ...
        If SaveGINKOManual() Then
            If txtsystem_lot_number.Text.Trim.Length > 0 Then
                ...
                If result = DialogResult.Yes Then DirectPrint()
            End If
            blnCancel = False
            Call Loaddata(txtsystem_lot_number.Text)
        Else
    ...
    Private Function SaveGINKOManual(Optional ByVal strGINNO As String = "") As Boolean
        ...
        Dim objDB As New classOperationKnittingNEW
        Dim Greige_Header As New classOperationKnittingNEW.Greige_Header
        Dim Defect_Roll_Header As New classOperationKnittingNEW.Defect_roll_Header
        ...
        Greige_Header.h18_mts = config.IsValidDouble(txtsecondary_quantity.Text.Trim) 'For Eschler 
        Greige_Header.h19_yds = config.IsValidDouble(txtsecondary_quantity.Text.Trim) / 0.9144 'For Eschler 
        Greige_Header.h20_kg_qc = config.IsValidDouble(txtprimary_quantity.Text.Trim) - config.IsValidDouble(txtbar_weight.Text.Trim)
        Greige_Header.h21_mts_qc = config.IsValidDouble(txtsecondary_quantity.Text.Trim) 'For Eschler 
        Greige_Header.h22_yds_qc = config.IsValidDouble(txtsecondary_quantity.Text.Trim) / 0.9144 'For Eschler 
        Greige_Header.h23_grade = txtlot_grade.Text.Trim
        ...
        'Greige_Header.h39_roll_no_o = IIf(txtcustom_lot_number.Text.Trim = "" Or txttran_no.Text.Trim = "", IIf(chkEsc.Checked, "N", IIf(chkGMK.Checked, "G", txtcustom_lot_number.Text.Trim)), txtcustom_lot_number.Text.Trim)
        'Greige_Header.h39_roll_no_o = IIf(txtcustom_lot_number.Text.Trim = "", IIf(chkEsc.Checked, "N", IIf(chkGMK.Checked, "G", txtcustom_lot_number.Text.Trim)), txtcustom_lot_number.Text.Trim) 'Sitthana 20210916
        If txtcustom_lot_number.Text.Trim = "" Then
            If chkEsc.Checked Then
                Greige_Header.h39_roll_no_o = "N"
            ElseIf chkGMK.Checked Then
                Greige_Header.h39_roll_no_o = "G"
            ElseIf chkFactoryRollGMKFomat.Checked Then
                Greige_Header.h39_roll_no_o = "NG" 'Sitthana 20210921
            Else
                Greige_Header.h39_roll_no_o = txtcustom_lot_number.Text.Trim
            End If
        Else
            Greige_Header.h39_roll_no_o = txtcustom_lot_number.Text.Trim
        End If 'Sitthana 20210916 - Greige_Header.h39_roll_no_o
        ...
        blnCancel = False
        Dim result As DialogResult 'Windows.Forms.DialogResult
        result = MessageBox.Show("Would you like to Gregie IN ?", "System Message", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button3)
    ...
    Private Sub dtpsteam_time_ValueChanged(sender As Object, e As EventArgs) Handles dtpsteam_time.ValueChanged

    End Sub

    Private Function ChkIntKey(ByVal pKeyPress As String)
        'KeyPressEventArgs Sitthana 20190128
        ...
        ElseIf Asc(pKeyPress) = Asc(".") Then
            ...
            MessageBox.Show("ให้ป้อนตัวเลข 0 - 9 และเป็นจำนวนเต็มเท่านั้น", "ข้อความเตือนการป้อนข้อมูล", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Else
            VL_ReturnValue = False
            MessageBox.Show("ให้ป้อนตัวเลข 0 - 9 และไม่มีจุดทศนิยม", "ข้อความเตือนการป้อนข้อมูล", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If

        Return VL_ReturnValue
    End Function
    'Private Sub txtdefect_details_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtdefect_details.KeyPress
    '    'Sitthana 20190128
    ...
    Private Sub chkEsc_Click(sender As Object, e As EventArgs) Handles chkEsc.Click
        If Trim(txtcustom_lot_number.Text) = "" Or Len(Trim(txtcustom_lot_number.Text)) <= 2 Then
            txtcustom_lot_number.Text = ""
        End If
    End Sub

    Private Sub chkGMK_Click(sender As Object, e As EventArgs) Handles chkGMK.Click
        If Trim(txtcustom_lot_number.Text) = "" Or Len(Trim(txtcustom_lot_number.Text)) <= 2 Then
            txtcustom_lot_number.Text = "G"
        End If
    End Sub

    Private Sub chkFactoryRollGMKFomat_Click(sender As Object, e As EventArgs) Handles chkFactoryRollGMKFomat.Click
        If Trim(txtcustom_lot_number.Text) = "" Or Len(Trim(txtcustom_lot_number.Text)) <= 2 Then
            txtcustom_lot_number.Text = "NG"
        End If
    End Sub

    'Private Sub txtMTSStart_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtMTSStart.KeyPress
    '    If ChkIntKey(e.KeyChar) = False Then
    '        e.KeyChar = Nothing
    '    End If
    'End Sub
    ...
    Private Sub txtBarNo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtBarNo.KeyPress
        If ChkIntKey(e.KeyChar) = False Then
            e.KeyChar = Nothing
        End If
    End Sub

    Private Sub txtGuideNo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtGuideNo.KeyPress
        If ChkIntKey(e.KeyChar) = False Then
            e.KeyChar = Nothing
        End If
    End Sub

    Private Sub tsbGreigeIn_Click_1(sender As Object, e As EventArgs) Handles tsbGreigeIn.Click
        Call SaveGreigeIN()
    End Sub

    Private Sub tsbPrintNew_Click(sender As Object, e As EventArgs) Handles tsbPrintNew.Click
        Call PrintBarcodeNew()
    End Sub
End Class