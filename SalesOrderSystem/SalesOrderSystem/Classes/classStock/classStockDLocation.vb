...
Public Class classStockDLocation
    ...
    'Conversion Project
    Public Function UpdateStockD(ByVal dt As DataTable, ByRef msgerr As String, ByRef UserID As String) As Boolean
        Dim config As New clsConfig
        Dim conn As New SqlConnection((New classConnection).connection)
        conn.Open()
        ...
        For j = 0 To dt.Rows.Count - 1
            With dt.Rows
                If dt.Rows(j).RowState <> DataRowState.Deleted Then
                    ...
                    comm.Parameters.AddWithValue("@mtl_warehouse_id", config.IsNull(.Item(j)("mtl_warehouse_id"), 0.0))
                    comm.Parameters.AddWithValue("@mtl_subinventory_id", config.IsNull(.Item(j)("mtl_subinventory_id"), 0.0))
                    comm.Parameters.AddWithValue("@mtl_locations_id", config.IsNull(.Item(j)("mtl_locations_id"), 0.0))
        ...
        Dim DistinctTable As DataTable = dt 'dv_dtc_upd.ToTable("strolls_d", True, "dinno")
        i = 0
        For i = 0 To DistinctTable.Rows.Count - 1
            With DistinctTable.Rows
                comm.Parameters.Clear()
                comm.Parameters.AddWithValue("@empcd", UserID.Trim)
                comm.Parameters.AddWithValue("@docno", config.IsNull(.Item(i)("dinno"), "").Trim)
