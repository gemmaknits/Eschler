...
Public Class frmSalesOrder
    ...
    Private _selectedBillToRow As DataRow
    Private _selectedShipToRow As DataRow
    ...
    Private Sub GenCombo()
        ...
        'Banks
        Dim dtBanks As DataTable = (New classMaster).comboBanks
        Me.mcboBanks.DataSource = dtBanks
    ...
    Private Function SaveData() As Boolean
        ...
        header.h03_custcd = If(_selectedBillToRow IsNot Nothing, _selectedBillToRow("custcd").ToString, "")
        ...
        header.h10_attn = ""
        header.h11_shipcustcd = If(_selectedShipToRow IsNot Nothing, _selectedShipToRow("custcd").ToString, "")
        ...
        header.h17_delicd = If(_selectedShipToRow IsNot Nothing, _selectedShipToRow("custcd").ToString, "")
    ...
    Private Sub BindCustomerBillToAddr()
        If _selectedBillToRow IsNot Nothing Then
            txtCustAddr.Text = _selectedBillToRow.Item(4).ToString
        End If
    End Sub
    Private Sub BindCustomerShipToAddr()
        If _selectedShipToRow IsNot Nothing Then
            txtDeliAddr.Text = _selectedShipToRow.Item(4).ToString
        End If
    End Sub
    Private Sub bindCustomerBillToData()
        If _selectedBillToRow Is Nothing Then Exit Sub
        Dim selectedCustCd As String = _selectedBillToRow("custcd").ToString.Trim
        Dim dt As DataTable = bsCustomersBillToFlag.DataSource
        For i As Integer = 0 To dt.Rows.Count - 1
            If dt.Rows(i)("custcd").ToString.Trim = selectedCustCd Then
                cboAgent.SelectedValue = dt.Rows(i)("new_agcd").ToString.Trim
                txtTermCondition.Text = dt.Rows(i)("new_payterms").ToString.Trim
                txtCreditDays.Text = dt.Rows(i)("new_credit").ToString.Trim
                txtDeliveryTerm.Text = dt.Rows(i)("deli").ToString.Trim
                txtCustAddr.Text = dt.Rows(i)("custaddr").ToString.Trim()
                txtContact.Text = dt.Rows(i)("contact").ToString.Trim
                mcboBanks.SelectedValue = dt.Rows(i)("id_banks")
                Exit For
            End If
        Next
    End Sub
    Private Sub bindCustomerShipToData()
        If _selectedShipToRow Is Nothing Then Exit Sub
        txtTransportBy.Text = _selectedShipToRow.Item(13).ToString.Trim
        txtAdditionalLabel.Text = _selectedShipToRow.Item(14).ToString.Trim
        txtDeliAddr.Text = _selectedShipToRow.Item(4).ToString.Trim
    End Sub
    ...
    Private Sub btnCustomerItems_Click(sender As Object, e As EventArgs) Handles btnCustomerItems.Click
        'Sitthana 12/02/2018
        If _selectedBillToRow Is Nothing Then
        ...
        With objfrmSearchSOCustItem
            objfrmSearchSOCustItem.CustomerNumber = _selectedBillToRow("custcd")
            objfrmSearchSOCustItem.CustomerName = _selectedBillToRow("name")
            If grdSalesOrder.Rows.Count > 1 Then
    ...
    Private Sub UpdateSelectedBillToRow()
        If mcboCustomersBillToFlag.SelectedIndex >= 0 Then
            Dim drv As DataRowView = TryCast(mcboCustomersBillToFlag.SelectedItem, DataRowView)
            _selectedBillToRow = If(drv IsNot Nothing, drv.Row, Nothing)
        Else
            _selectedBillToRow = Nothing
        End If
    End Sub

    Private Sub UpdateSelectedShipToRow()
        If mcboCustomersShipToFlag.SelectedIndex >= 0 Then
            Dim drv As DataRowView = TryCast(mcboCustomersShipToFlag.SelectedItem, DataRowView)
            _selectedShipToRow = If(drv IsNot Nothing, drv.Row, Nothing)
        Else
            _selectedShipToRow = Nothing
        End If
    End Sub
    ...
    Private Sub mcboCustomersBillToFlag_SelectedIndexChanged(sender As Object, e As EventArgs) Handles mcboCustomersBillToFlag.SelectedIndexChanged
        UpdateSelectedBillToRow()
        Call bindCustomerBillToData()
        Call BindCustomerBillToAddr()

        'Begin - Sitthana 19/09/2018
        If _selectedBillToRow IsNot Nothing Then
            Dim activeFlag As String = _selectedBillToRow.Item(17).ToString
    ...
    Private Sub mcboCustomersShipToFlag_SelectedIndexChanged(sender As Object, e As EventArgs) Handles mcboCustomersShipToFlag.SelectedIndexChanged
        UpdateSelectedShipToRow()
        Call bindCustomerShipToData()
        Call BindCustomerShipToAddr()
    End Sub

    Private Sub btnBillToLookup_Click(sender As Object, e As EventArgs) Handles btnBillToLookup.Click
        Dim frm As New frmLookup(dtCustomersBillToFlag,
            New List(Of String) From {"custcd", "name", "custaddr"},
            New List(Of String) From {"Code", "Customer Name", "Address"})
        If frm.ShowDialog() = DialogResult.OK Then
            _selectedBillToRow = frm.SelectedRow
            mcboCustomersBillToFlag.SelectedValue = _selectedBillToRow("custcd")
        End If
        frm.Dispose()
    End Sub

    Private Sub btnShipToLookup_Click(sender As Object, e As EventArgs) Handles btnShipToLookup.Click
        Dim dv As New DataView(dtCustomersShipToFlag)
        If _selectedBillToRow IsNot Nothing Then
            dv.RowFilter = "parent_customer_id = " & _selectedBillToRow.Item(0).ToString
        End If
        Dim frm As New frmLookup(dv.ToTable(),
            New List(Of String) From {"custcd", "name", "custaddr"},
            New List(Of String) From {"Code", "Customer Name", "Address"})
        If frm.ShowDialog() = DialogResult.OK Then
            _selectedShipToRow = frm.SelectedRow
            mcboCustomersShipToFlag.SelectedValue = _selectedShipToRow("custcd")
        End If
        frm.Dispose()
    End Sub

    Private Sub btnBanksLookup_Click(sender As Object, e As EventArgs) Handles btnBanksLookup.Click
        Dim frm As New frmLookup(CType(mcboBanks.DataSource, DataTable),
            New List(Of String) From {"bank_display"},
            New List(Of String) From {"Bank"})
        If frm.ShowDialog() = DialogResult.OK Then
            mcboBanks.SelectedValue = frm.SelectedRow("id_banks")
        End If
        frm.Dispose()
    End Sub

    Private Sub btnDesignPropertiesLookup_Click(sender As Object, e As EventArgs) Handles btnDesignPropertiesLookup.Click
        Dim frm As New frmLookup(CType(mcboDesignProperties.DataSource, DataTable),
            New List(Of String) From {"lookup_value_code"},
            New List(Of String) From {"Design Property"})
        If frm.ShowDialog() = DialogResult.OK Then
            mcboDesignProperties.SelectedValue = frm.SelectedRow("lookup_value_id")
        End If
        frm.Dispose()
    End Sub
