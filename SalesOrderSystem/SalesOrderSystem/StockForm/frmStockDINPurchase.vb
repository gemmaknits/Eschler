...
Public Class frmStockDINPurchase
    ...
    Dim oStockDINPurchase As New classStockDINPurchase
    ...
    Private Sub InitControl()
        ...
        grdData.DataSource = (New classStockDINPurchase).GetDINPurchase("0", clsUser.UserID)
    ...
    Private Sub GenCombo()
        ...
        cboMtlWarehouse.DataSource = clsMaster.Combomtlwarehouse("")
        cboMtlWarehouse.DisplayMember = "warehouse_code"
        cboMtlWarehouse.ValueMember = "mtl_warehouse_id"
        cboMtlWarehouse.SelectedValue = clsMaster.GetdefaultWarehouseID("")

        'cboMtlSubinventory.DataSource = (New classMaster).ComboMtlsubinventory(cboMtlWarehouse.SelectedValue)
        'cboMtlSubinventory.DisplayMember = "subinventory_code"
        'cboMtlSubinventory.ValueMember = "mtl_subinventory_id"

        'cboMtlLocation.DataSource = (New classMaster).Combomtllocations("", cboMtlWarehouse.SelectedValue, cboMtlLocation.SelectedValue)
    ...
    Private Sub GenComboDINNo()
        ...
        cboDinNo.ComboBox.DataSource = objDB.GetDINPurchaseNo(pStockType)
    ...
    Private Function GetColNo(ByVal strColNo As String) As Boolean
        ...
        Dim dt As DataTable = oStockDINPurchase.GetColItem(strColNo, clsUser.UserID)
    ...
    Private Sub cboDinNo_DropDownClosed(sender As Object, e As System.EventArgs) Handles cboDinNo.DropDownClosed
        If oConfig.IsNull(cboDinNo.ComboBox.SelectedValue, "").ToString.Length = 0 Then Exit Sub
        Call getDINPurchase(oConfig.IsNull(cboDinNo.ComboBox.SelectedValue, "").ToString)

    End Sub
    Function getDINPurchase(ByRef strDINNO As String) As Boolean

        Dim dt As DataTable = oStockDINPurchase.GetDINPurchase(strDINNO, clsUser.UserID)
        If dt.Rows.Count > 0 Then
    ...
    Private Sub BindData(ByVal dt As DataTable)
        ...
        txtPONo.Text = (New clsConfig).IsNull(dt.Rows(0)("purno"), "").ToString.Trim()
    ...
    Private Function CheckData() As Boolean
        If oConfig.IsNull(cboMtlWarehouse.SelectedValue, 0) = 0 Then
            ...
            Exit Function
        Else
            ErrorProvider1.SetError(cboMtlWarehouse, "")
        End If

        If oConfig.IsNull(cboMtlSubinventory.SelectedValue, 0) = 0 Then
        ...
        For Each row As DataGridViewRow In grdData.Rows
            If Not (txtPONo.Text.Trim = ("FREE SAMPLE") Or txtPONo.Text.Trim = ("FREE OF CHARGE")) Then
                If (New clsConfig).IsNull(row.Cells("id_jobdet").Value, 0) = 0 Then
    ...
    Private Function SaveDINPurchase() As Boolean
        ...
        Din_header.h36_loc = cboMtlLocation.Text
        ...
        Din_header.h56_mtl_warehouse_id = cboMtlWarehouse.SelectedValue
        Din_header.h57_mtl_subinventory_id = cboMtlSubinventory.SelectedValue
        Din_header.h58_mtl_locations_id = cboMtlLocation.SelectedValue
        Din_header.h59_purno = txtPONo.Text.Trim
        Din_header.h60_suppcd = oConfig.IsNull(cboSupplier.SelectedValue, Nothing)
        Din_header.h61_sinvno = txtSuppInvNo.Text
        Din_header.h62_sinvdt = IIf(dtpsinvdt.Checked, dtpsinvdt.Value, Nothing)
        Din_header.h63_lotno_sup = txtSuppLot.Text.Trim
        Din_header.h64_srefno = txtSuppRefNo.Text.Trim
        ...
        For i = 0 To dtc.Rows.Count - 1
            If dtc.Rows(i).RowState <> DataRowState.Deleted Then
                ...
                dtc.Rows(i)("suppcd") = oConfig.IsNull(cboSupplier.SelectedValue, Nothing)
                dtc.Rows(i)("sinvno") = txtSuppInvNo.Text.Trim
                dtc.Rows(i)("sinvdt") = If(dtpsinvdt.Checked, dtpsinvdt.Value, DBNull.Value)
        ...
        If objDB.DinPurchaseSave(Din_header, msgerr, dtc, dv_dtc_add, dv_dtc_upd, dv_dtc_del, clsUser.UserID, DINNo, pStockType) Then
            StrDinno = DINNo
            Call GenComboDINNo()
            Call GenCombo()
            Call getDINPurchase(oConfig.IsNull(StrDinno, ""))
    ...
    Private Sub grdData_CellValueChanged(sender As Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdData.CellValueChanged
        ...
        If grd.Columns(e.ColumnIndex).Name = "col" Then
            Dim StrCol As String
            StrCol = grd.Rows(e.RowIndex).Cells("col").Value
            grd.Rows(e.RowIndex).Cells("custcolor").Value = obj.GetCustCol(StrCol)
        End If
    ...
    Private Sub tsbDInDocument_Click(sender As Object, e As EventArgs) Handles tsbDInDocument.Click
        Dim clsConfig As New clsConfig
        Dim rptFileName As String = "rptDIN.rpt"
