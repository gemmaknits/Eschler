...
namespace PDR.BLL
{
    class classDR_Final_Response_BLL
    {
        ...
        public DataSet Populate_APPREJ()
        {
            ...
            SqlCommand command1 = (SqlCommand)db.GetCommandToSetParameters("[PDR].[p_pdr_dr_combo_apprej_final]");
            try
            {
        ...
        public DataTable UpdateDRNODetails(string p_dr_no, string p_dr_app_rej_id, string p_dr_app_rej_comment, string p_user_session_id)
        {
            ConnectionDB db = ConnectionDB.getDbInstance();
            db.ConnectToDB();
            SqlCommand command1 = (SqlCommand)db.GetCommandToSetParameters("[PDR].[p_pdr_dr_apprej_final_update]");
            try
            {
        ...
        public string sendFinalApprovalMail(DataTable dt, string attachmentIDs, string sessionID, string tomaillist, string ccmaillist, string subject, string bodytext)
        {
            string strResult;
            try
            {
                ...
                else
                {
                    ...
                    //CCmailList = Convert.ToString(r["rd_staff_email"]) + "," + Convert.ToString(r["requester_email"]) + "," + Convert.ToString(r["preparer_email"]) + "," + Convert.ToString(r["sample_room_Email"]) + "," + Convert.ToString(r["factory_manager_email"]) + "," + Convert.ToString(r["md_email"]);
                }
               // CCmailList = "kalai@eplt.in" + "," + "suresh@geminitextile.com";
                ...
                if (bodytext != "")
                {
                    ...
                    msgbody = msgbody.Replace("FINAL APP/REJ:  " + dt.Rows[0]["DR_FINAL_aPP_REJ_CODE"].ToString().Trim(), "");
                    msgbody = msgbody.Replace("FINAL COMMENT:  " + dt.Rows[0]["DR_FINAL_aPP_REJ_COMMENT"].ToString().Trim(), "");
                    ...
                    strBodyHTML.Append("<BR>");
                    strBodyHTML.Append("FINAL APP/REJ:  " + dt.Rows[0]["DR_CUSTOMER_aPP_REJ_CODE"].ToString().Trim() + "     ");
                    strBodyHTML.Append("<BR>");
                    strBodyHTML.Append("FINAL COMMENT:  " + dt.Rows[0]["DR_FINAL_aPP_REJ_COMMENT"].ToString().Trim() + "     ");
                    strBodyHTML.Append("<BR>");
                    ...
                    msg.Subject = dt.Rows[0]["final_RESPONSE_subject"].ToString();
                }
                else
                {
                    strBodyHTML.Append("<HTML>");
                    strBodyHTML.Append("<H1><Td color='blue'>FINAL APPROVAL</td></h1>");
        ...
        public string sendFinalApproveMail(DataTable dt, string attachmentIDs, string sessionID)
        {
            string strResult;
            try
            {
                ...
                CCmailList = "kalai@eplt.in" + "," + "suresh@geminitextile.com";
                CCmailList = Convert.ToString(r["requester_email"]) + "," + Convert.ToString(r["preparer_email"]) + "," + Convert.ToString(r["sample_room_Email"]) + "," + Convert.ToString(r["factory_mgr"]) + "," + Convert.ToString(r["md"]);
                toEmailID = Convert.ToString(r["rd_staff"]);
                string[] CCId = CCmailList.Split(',');
                MailAddress toMailId = new MailAddress(toEmailID);
                ...
                strBodyHTML.Append("DR No. " + dt.Rows[0]["pdr_no"] + "       ");
                strBodyHTML.Append("</a>");
                strBodyHTML.Append("<BR>");
                strBodyHTML.Append("CUSTOMER APP/REJ:  " + dt.Rows[0]["pdr_date"] + "     ");
                strBodyHTML.Append("<BR>");
                strBodyHTML.Append("ADD TO COLLECTION:  " + dt.Rows[0]["pdr_date"] + "     ");
                strBodyHTML.Append("<BR>");
                strBodyHTML.Append("CUSTOMER COMMENT:  " + dt.Rows[0]["pdr_date"] + "     ");
                strBodyHTML.Append("<BR>");
