<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" EnableEventValidation="false" AutoEventWireup="true" CodeBehind="DR_Sample_Dispo.aspx.cs" Inherits="PDR.UI.DR_Sample_Dispo" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div style="position: absolute; top: 90px; left: 10px">
        <h4>DR KNITTING SAMPLE DISPO</h4>
    </div>
    <div style="position: absolute; top: 140px; left: 5px">
        ...
        <table>
            <tr>
                <td>
                    <asp:GridView ID="grdDRSampleKnitting" runat="server" AutoGenerateColumns="false" GridLines="None"
                        EmptyDataText="There is No Records To Display" Width="460px" DataKeyNames="mfg_sample_dispo_id,app_rej_comment"
                        OnRowDataBound="gvorder_issues_OnRowDataBound" OnSelectedIndexChanged="OnSelectedIndexChanged"
                        CssClass="table">
                        <HeaderStyle CssClass="grdheader" />
                        <SelectedRowStyle BackColor="#FFF3B7" ForeColor="Maroon" Font-Bold="True" />
                        ...
                        <Columns>
                            ...
                            <asp:BoundField DataField="dispo_Seq" HeaderStyle-Width="2px" HeaderText="#" HeaderStyle-CssClass="col col-xs-2" ItemStyle-CssClass="col col-xs-2" HeaderStyle-HorizontalAlign="Right" HeaderStyle-Font-Bold="false" ReadOnly="True">
                            ...
                            <asp:BoundField DataField="CREATION_DATE" HeaderStyle-Width="80px" HeaderText="DATE" DataFormatString="{0:d}" HeaderStyle-CssClass="co" ItemStyle-CssClass="col" HeaderStyle-HorizontalAlign="Right" HeaderStyle-Font-Bold="false" ReadOnly="True"></asp:BoundField>
                            ...
                            <asp:TemplateField HeaderStyle-Width="130px" HeaderText="COMMENT" HeaderStyle-CssClass="col" ItemStyle-CssClass="col" HeaderStyle-Font-Bold="false">
                                <ItemTemplate>
                                    <asp:TextBox ID="SAMPLEDISPOCOMMENT" runat="server" TextMode="MultiLine" Height="40px" onkeydown="(event.keyCode!=13);" onkeypress="(event.keyCode!=13);" Text='<%# Eval("sample_dispo_comment") %>' CssClass="TextBox" Width="130px" />
                                </ItemTemplate>
                            </asp:TemplateField>
                            <asp:BoundField DataField="APP_REJ_VALUE" HeaderText="APP/REJ" HeaderStyle-CssClass="col col-xs-2" ItemStyle-CssClass="col col-xs-2" HeaderStyle-HorizontalAlign="Right" HeaderStyle-Font-Bold="false" ReadOnly="True">
                                <ItemStyle CssClass="col" Width="20px" />
                            </asp:BoundField>
                            <asp:BoundField DataField="APP_REJ_COMMENT" HeaderText="APP/REJ COMMENT" HeaderStyle-CssClass="col col-xs-2" ItemStyle-CssClass="col col-xs-2" HeaderStyle-HorizontalAlign="Right" HeaderStyle-Font-Bold="false" ReadOnly="True">
                                <ItemStyle CssClass="col" Width="20px" />
                            </asp:BoundField>
                        </Columns>
                    </asp:GridView>
    ...
    <script type="text/javascript">
        $(function () {
            $("[id*=grdDRSampleKnitting] td").click(function () {
        ...
        function DisplayDetails(row) {
           ...
           // alert($('#ContentPlaceHolder1_ContentPlaceHolder1_grdDRSampleKnitting_SAMPLEDISPOCOMMENT_' + row).val());
            //alert(seq);
            //alert(comment);
    ...
    <script>
        function load_Data(sampledispoID) {
            $.ajax({
                type: "POST",
                url: "DR_Sample_Dispo.aspx/GetData",
        ...
        //    $.ajax({
        //        type: "POST",
        //        url: "DR_Sample_Knitting.aspx/GetData2",
        //        data: "{'source_doc_no':'" + sampledispoID + "'}",
        //        contentType: "application/json; charset=utf-8",
        //        dataType: "json",
        //        success: OnSuccess,
        ...
        //        //alert(customers);
        //        // $("#ContentPlaceHolder1_gvyarnstock").empty();
        //        var row = $("[id*=grdAttachmentDetails] tr:last-child").clone(true);
        //        $("[id*=grdAttachmentDetails] tr").not($("[id*=grdAttachmentDetails] tr:first-child")).remove();
        //        $.each(customers, function () {
        //            var customer = $(this);
        //            $("td", row).eq(0).html($(this).find("file_description").text());
        //            $("td", row).eq(1).html($(this).find("file_location_disp").text());
        //            $("[id*=grdAttachmentDetails]").append(row);
        //           // $("#ContentPlaceHolder1_grdAttachmentDetails").append(row);
        //            row = $("[id*=grdAttachmentDetails] tr:last-child").clone(true);
        //            //alert("YES");
        //        });
        //    }
   ...
   <script type="text/javascript">
       function smaoledownload(grdattachment) {
          ...
          // alert(attachmentid);
           ...
           //$.download('DR_Sample_Knitting.aspx/downloadAttachment', 'file_location=' + file_location + 'sourcedocno=' + attachmentid);
           //return false;


           //$(function () {
           //    $("[id*=grdAttachmentDetails] td").click(function () {
           //         DisplayDetails_grdAttachmentDetails($(this).closest("tr"));
           //    });
           //});
           //function DisplayDetails_grdAttachmentDetails(row) {
           //    //alert(row);
           //    var yarncode = "";
           //    var pdfno = "";
           //    yarncode = $("td", row).eq(2).html();
           //    // yarncode = $.trim(yarncode).replace('<a style="text-decoration:underline;cursor:pointer;color:Blue;">', '').replace('</a>', '').trim();
           //    alert(yarncode);
           //    var designno = $('#txtdesignno').val();
           //    //alert(designno);
           //    // message += "\nName: " + $("td", row).eq(1).html();
           //    //pdfno = "\nDescription: " + $("td", row).eq(2).html();
           //    // pdfno = $("td", row).eq(2).html();
           //    // var leftgridpdfqty = $("td", row).eq(4).html();
           //    // var designno = $('#txtdesignno').val();
           //    // alert(designno);
           //    var yarnfactorydesignno = $('#txtFactoryDesign').val();
           //    var dataname = "YARN-SELECTED-ITEM";
           //    var left = "200px";// (screen.width / 2) - (popupWidth / 2);
           //    var top = "200px";//(screen.height / 2) - (popupHeight / 2);
           //    var pageURL = "YarnStock_Avail_Details.aspx?finisheddesignno=" + designno + "&yarnfactorydesignno=" + yarnfactorydesignno + "&yarncode=" + yarncode + "&dataname=" + dataname;
           //    //var popupWidth = "50%";
           //    //  var popupHeight = "150%";
           //    var targetPop = window.open(pageURL, '', 'toolbar=no, location=no, directories=no,status=no, menubar=no, scrollbars=YES, resizable=YES, copyhistory=no,width=1200, height=300, top=150, left=50');
           //    //  alert(leftgridpdfqty);
           //    //  DisplayDetailsRight(leftgridpdfqty);
           //    //var left_poc_header_id =$('table td:eq(10;)');// $("td", row).eq(10).html();
           //    //alert(left_poc_header_id.text);
           //    //var rowindexpdf = $(this).closest('td').parent()[0].sectionRowIndex;
           //    //alert(rowindexpdf);
           //    // greigevalue(pdfno);
          // }
       }
    ...
    <script type="text/javascript">
        ...
        function ChangeRowColor(row, rowIndex, poc_header_id) {
            ...
            $('#<%= grdDRSampleKnitting.ClientID %>').val(row);
            $('#<%= grdDRSampleKnitting.ClientID %>').val(rowIndex);
        }
        ...
        function RetainSelectedRow() {
            var parent = $('#<%= grdDRSampleKnitting.ClientID %>').val();
            var currentIndex = $('#<%= grdDRSampleKnitting.ClientID %>').val();
            if (parent != null) {
