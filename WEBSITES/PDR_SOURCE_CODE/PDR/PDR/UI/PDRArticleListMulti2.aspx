        ...
        <div class="row">
            <div class="col-xs-12" style="padding-left: 0px !important; padding-right: 0px !important; overflow: hidden !important; height: 100%;">
                ...
                <div class="panelnew panel-primary">
                            ...
                            <table >
                                ...
                                <tr style="line-height: 5px !important;">
                                    ...
                                    <td style="white-space:nowrap;">
                                        <asp:Button ID="Button1" runat="server" Text="Find" CssClass="btn btn-success btnfont" OnClick="btnFind_Click" />
                                        &nbsp;
                                        <asp:Button ID="Button2" runat="server" Text="Reset All" CssClass="btn btn-success btnfont" OnClick="btnClear_Click" />
                                        &nbsp;
                                        <asp:Button ID="Button3" runat="server" Text="Excel" CssClass="btn btn-success btnfont" OnClick="btnExcel_Click" />
                                    </td>
                                </tr>
                            </table>
                               </div>
                    ...
                    <div class="panel-heading" style="display:flex; align-items:center; height:auto !important; padding:5px 15px;">
                        <span class="panel-title" style="margin-right:20px; white-space:nowrap;">Article Filter</span>
                        <div style="position:relative;">
                            <input type="text" id="txtFilterAll" onkeyup="filterAllSearch(this)" onfocus="onFilterFocus(this)"
                                   placeholder="Search all filters..." autocomplete="off" class="combo"
                                   style="width:280px; height:23px; color:#333;" />
                                <div id="filterDropdown" style="display:none; position:absolute; background:#fff; 
                                     border:1px solid #999; max-height:350px; overflow-y:auto; 
                                     width:420px; z-index:9999; font-size:11px; top:25px; left:0px; color:#333;">
                            </div>
                        </div>
                    </div>
...
<!-->                                    <asp:Label class="LabelWide GVTextBoxBorder" ID="txtDesign2" runat="server"  Text='<%# Bind("design_no") %>'></asp:Label> -->
 ...
 <%--                           <asp:TemplateField HeaderText="MIN Bht/KG" HeaderStyle-CssClass="header-center" >
    ...
    <script type="text/javascript">
      ...
      <script type="text/javascript">
            var cblData = [
                { id: '<%= cblAppl.ClientID %>', name: 'APPLICATION' },
                { id: '<%= cblSubAppl.ClientID %>', name: 'SUB APPLICATION' },
                { id: '<%= cblSplFunc.ClientID %>', name: 'SPL. FUNC' },
                { id: '<%= cblDesignType.ClientID %>', name: 'DESIGN TYPE' },
                { id: '<%= cblFamilyName.ClientID %>', name: 'FAMILY' },
                { id: '<%= cblFinishing.ClientID %>', name: 'FINISHING' },
                { id: '<%= cblFibreType.ClientID %>', name: 'FIBRE' },
                { id: '<%= cblFibreSubType.ClientID %>', name: 'KIND OF YARN' }
            ];
          var ddlData = [
                { id: '<%= ddlcategory.ClientID %>', name: 'CATEGORY' },
                { id: '<%= ddlSubCategory.ClientID %>', name: 'SUB CATEGORY' },
                { id: '<%= ddlGroup.ClientID %>', name: 'FABRIC GROUP' },
                { id: '<%= ddlSubGroup.ClientID %>', name: 'FABRIC SUB GROUP' }
            ];

          function buildDropdown(searchText) {
              var dropdown = document.getElementById('filterDropdown');
              var header = '<div style="padding:4px 8px; border-bottom:1px solid #ccc; background:#f0f0f0;">' +
                           '<a href="#" onclick="selectAllDropdown(true); return false;" ' +
                           'style="font-size:10px; font-family:Segoe UI,Arial,sans-serif; margin-right:12px; color:#333;">Select All</a>' +
                           '<a href="#" onclick="selectAllDropdown(false); return false;" ' +
                           'style="font-size:10px; font-family:Segoe UI,Arial,sans-serif; color:#333;">Unselect All</a>' +
                           '</div>';
              var itemsHtml = '';
              var found = false;

              for (var c = 0; c < cblData.length; c++) {
                  var cbl = document.getElementById(cblData[c].id);
                  if (!cbl) continue;
                  ...
                  for (var i = 0; i < rows.length; i++) {
                      ...
                      if (label && chk) {
                          ...
                          if (searchText === '' || labelText.toLowerCase().indexOf(searchText) > -1) {
                              found = true;
                              var isChecked = chk.checked ? 'checked' : '';
                              itemsHtml += '<div style="padding:4px 8px; cursor:pointer; font-size:10px; font-family:Segoe UI,Arial,sans-serif; color:#333;" ' +
                                           'onclick="toggleDropdownItem(this)" ' +
                                           'onmouseover="this.style.background=\'#e8f4e8\'" ' +
                                           'onmouseout="this.style.background=\'#fff\'">' +
                                           '<input type="checkbox" data-orig="' + chk.id + '" ' + isChecked +
                                           ' onclick="event.stopPropagation();" onchange="syncCheckbox(this)" style="margin-right:5px;" />' +
                                           labelText +
                                           '<span style="color:#888; font-size:10px; margin-left:5px;">(' + cblData[c].name + ')</span>' +
                                           '</div>';
                          }
                      }
                  }
              ...
              for (var d = 0; d < ddlData.length; d++) {
                  var ddl = document.getElementById(ddlData[d].id);
                  if (!ddl) continue;
                  for (var j = 0; j < ddl.options.length; j++) {
                      ...
                      if (searchText === '' || labelText.toLowerCase().indexOf(searchText) > -1) {
                          found = true;
                          var isChecked = (ddl.value === opt.value) ? 'checked' : '';
                          itemsHtml += '<div style="padding:4px 8px; cursor:pointer; font-size:10px; font-family:Segoe UI,Arial,sans-serif;" ' +
                                       'onclick="toggleDropdownItem(this)" ' +
                                       'onmouseover="this.style.background=\'#e8f4e8\'" ' +
                                       'onmouseout="this.style.background=\'#fff\'">' +
                                       '<input type="checkbox" data-ddl="' + ddlData[d].id + '" data-val="' + opt.value + '" ' + isChecked +
                                       ' onclick="event.stopPropagation();" onchange="syncCheckbox(this)" style="margin-right:5px;" />' +
                                       labelText +
                                       '<span style="color:#888; font-size:10px; margin-left:5px;">(' + ddlData[d].name + ')</span>' +
                                       '</div>';
                      }
                  }
              }

              dropdown.innerHTML = header + (found ? itemsHtml : '<div style="padding:5px; color:#888; font-size:10px;">No results found</div>');
              dropdown.style.display = 'block';
          }

            function filterAllSearch(txt) {
                buildDropdown(txt.value.toLowerCase().trim());
            }
            ...
            function syncCheckbox(chk) {
                if (chk.hasAttribute('data-orig')) {
                    var original = document.getElementById(chk.getAttribute('data-orig'));
                    if (original) original.checked = chk.checked;
                } else if (chk.hasAttribute('data-ddl')) {
                    var ddl = document.getElementById(chk.getAttribute('data-ddl'));
                    if (ddl) ddl.value = chk.checked ? chk.getAttribute('data-val') : '';
                }
            }

            function toggleDropdownItem(div) {
                var chk = div.getElementsByTagName('input')[0];
                if (chk) {
            ...
            function selectAllDropdown(checked) {
                var dropdown = document.getElementById('filterDropdown');
                var checkboxes = dropdown.getElementsByTagName('input');
                for (var i = 0; i < checkboxes.length; i++) {
                    if (checkboxes[i].type === 'checkbox') {
                        checkboxes[i].checked = checked;
                        var original = document.getElementById(checkboxes[i].getAttribute('data-orig'));
            ...
            document.addEventListener('click', function (e) {
                var dropdown = document.getElementById('filterDropdown');
                var txt = document.getElementById('txtFilterAll');
                if (dropdown && txt && e.target !== txt && !dropdown.contains(e.target)) {
