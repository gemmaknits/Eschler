...
Public Class GetDataYarn
    ...
    Public Function GetYarnInno(ByVal YarnIn As String, ByVal Itemcode As String, ByVal pKoNo As String,
    ...
    Public Function getItemDataFromKI(ByVal paramKono As String) As DataSet
        ...
        Return ds
    End Function
    Public Function getKOYarnDetails(ByVal paramKono As String, Optional ByVal paramscrap_date As Nullable(Of Date) = Nothing) As DataSet
        Dim conn As New SqlConnection(connStr.Connection())
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_ko_yarn_select2"
        comm.Parameters.AddWithValue("@kono", paramKono)
        comm.Parameters.AddWithValue("@scrap_date", paramscrap_date)
        Dim da As New SqlDataAdapter(comm)
        Dim ds As New DataSet
        da.Fill(ds, "ko_yarn")
        conn.Close()  'Sitthana 20190325
    ...
    Public Function getProductionItem(kono As String) As DataTable
        ...
        Return dt
    End Function
    Public Function getGB() As DataTable
        Dim conn As New SqlConnection((New classConnection).Connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_get_gb_item"
        'comm.Parameters.AddWithValue("@kono", kono)
        Dim da As New SqlDataAdapter(comm)
        Dim dt As New DataTable
        da.Fill(dt)
    ...
    'Public Function GetDefaultWareHouse() As Nullable(Of Int64)
    ...
    Public Function GetdefaultmtlsubinventoryByItcd(ByVal itcd As String, ByVal strUSerID As String) As Int64
        Dim conn As New SqlConnection((New classConnection).Connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        comm.CommandText = "p_combo_master_mtl_subinventory_set_default_by_itcd"
    ...
    Public Function Getdefaultmtl_locations_id(ByVal Int64mtl_warehouse_id As Int64, ByVal Int64mtl_subinventory_id As Int64, ByVal strUSerID As String) As Int64
        Dim conn As New SqlConnection((New classConnection).Connection)
        Dim comm As New SqlCommand("", conn)
        comm.CommandType = CommandType.StoredProcedure
        ...
        da.Fill(dt)
        conn.Close()  'Sitthana 20190325
        Return dt.Rows(0)("mtl_locations_id")
    End Function
